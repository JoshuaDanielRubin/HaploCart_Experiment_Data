...
class Phylotree(object):
    ...
    def read_csv(self, phy_in):
        ...
        for line in phy_in:
            ...
            if "M4" in hap_id and "67" in hap_id:
                    hap_id == "M4''67"
...
def main():
    f = open("../data/children.txt", "wt")
    child_writer = csv.writer(f, delimiter="\t")

    g = open("../data/parents.txt", "wt")
    parent_writer = csv.writer(g, delimiter="\t")

    h = open("../data/unstable.txt", "wt")
    unstable_writer = csv.writer(h, delimiter="\t")
    ...
    if len(sys.argv) > 1:
        ...
        with open(phy_fn, 'r') as phy_in:
            ...
            for node in phy.nodes:
                for var in node.variants:
                    if is_unstable(var):
                        unstable_set.add(pos_from_var(var) + 1)

            for u in list(unstable_set):
                unstable_writer.writerow([u])
            quit()
            ...
            for node in phy.nodes:
                ...
                while node.parent is not None:
                    if "[" not in node.parent.hap_id:
                        parents.append(node.parent.hap_id)
                    else:
                        if "[" not in node.parent.parent.hap_id:
                            parents.append(node.parent.parent.hap_id)
                        else:
                            if "[" not in node.parent.parent.parent.hap_id:
                                parents.append(node.parent.parent.parent.hap_id)
                            else:
                                parents.append(node.parent.parent.parent.parent.hap_id)
                    node = node.parent
                parents.append("mt-MRCA")

                to_write = []
                for parent in parents:
                    if "M4" in parent and "67" in parent:
                        if "[" not in parent:
                            to_write.append("M4''67")
                        else:
                             to_write.append("M4''67[1]")
                    else:
                        to_write.append(parent)
                parent_writer.writerow(parents)

            child_writer.writerow(["mt-MRCA", "L0", "L1'2'3'4'5'6"])
            for node in phy.nodes:
                #if "]" in node.hap_id:
                #    continue
                children = [node.hap_id]
                for child in node.children:
                    children.append(child.hap_id)
                    if "[" not in child.hap_id:
                        children.append(child.hap_id)
                    else:
                        for child_ in child.children:
                            if "[" not in child_.hap_id:
                                children.append(child_.hap_id)
                            else:
                                for child__ in child_.children:
                                    if "[" not in child__.hap_id:
                                        children.append(child__.hap_id)
                                    else:
                                        for child___ in child__.children:
                                            children.append(child___.hap_id)

                to_write = []
                for child in children:
                    if "M4" in child and "67" in child:
                        if "[" not in child:
                            to_write.append("M4''67")
                        else:
                            to_write.append("M4''67[1]")
                ...
                child_writer.writerow(to_write)
